...
function initialise_pcompare_table(tableName, lowpoint, midpoint, highpoint){
    ...
    let table = $(tName).DataTable({
      ...
      drawCallback: function(settings){
          ...
          $('.NotDetected2').tooltip({title: "This MS peak was not detected for this age/life stage combination", placement: "top", position:"relative"})
      },
...
$(document).ready(function() {
  ...
  try {
    for (var i = 0; i < num_metabolites; i++) {
      ...
      title="${pathway_name} changes in FlyMet aged flies" target="_blank">${pathway_name} in FlyMet</a>`);
