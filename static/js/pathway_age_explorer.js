...
function initialise_pals_table(tableName, lowpoint, midpoint, highpoint){
    ...
    const MIN_VAL = 3000;
    const pals_age_data = document.getElementById('pals_age_data').getAttribute('url');
...
function updatePathwaySidePanel(obj){
  ...
  let reactome_id = $('#pals_age_data').DataTable().row(currentRow).data()[0];
  let pathway_name = $('#pals_age_data').DataTable().row(currentRow).data()[1];
...
$(document).ready(function() {
    ...
    let pals_table = initialise_pals_table("pals_age_data", min_value, mean_value, max_value);
