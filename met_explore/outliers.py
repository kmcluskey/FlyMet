...
class Outliers(object):
    ...
    that are higher or lower than the other members in the set (column_set).
    """
    ...
    def get_id_boxplot(self, sample_name, savefig=True):
        """
        ...
        # Add the label Mkr (Marker( at the intenisty position for each metabolite for the sample being analysed.
        ...
        for name in name_list:
            try:
                ...
                try:
                    ...
                    pass
        if cmpd_id == None:
            print(" %s cannot be found in the DB returning %d " % (cmpd_name, cmpd_id))
        else:
            print("returning %d for %s" % (cmpd_id, cmpd_name))
