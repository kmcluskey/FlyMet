...
class CompoundSelector(object):
    ...
    def construct_hc_int_df(self, high_conf_df):
        ...
        try:
            ...
            peaks = Peak.objects.filter(project=self.project, psec_id__in=sec_ids)
            ...
            for p in tqdm(peaks):
                ...
                cmpd = Compound.objects.get(pc_sec_id=cmpd_id, chebi_id=chebi_id, project=self.project)
    ...
    def construct_cmpd_intensity_df(self):
        ...
        samples = Sample.objects.all()
        peaks = Peak.objects.filter(project=self.project)
        df_index = [p.id for p in peaks]
    ...
    def get_single_cmpd_df(self, hc_int_df):
        ...
        for dc in duplicate_compounds:
            ...
            dup_peaks = Peak.objects.filter(project=self.project, id__in=dup_peak_list)
            max_values_dict = {}
            for dp in dup_peaks:
    ...
    def get_compound_details(self, peak_id, cmpd_id):
        ...
        peak = Peak.objects.get(id=peak_id, project=self.project)
        cmpd = Compound.objects.get(id=cmpd_id, project=self.project)
    ...
    def add_preferred_annotations(self, single_cmpds_df):
        ...
        for pid, cid in zip(peaks_ids, cmpd_ids):
            peak = Peak.objects.get(id=pid, project=self.project)
    ...
    def update_std_cmpds(self):
        ...
        cmpds = Compound.objects.filter(project=self.project)
        ...
        for c in cmpds:
            ...
            if len(all_ids) == 1 and 'stds_db' in all_ids:  # If the cmpd has a std Id and no other DB identifiers
                ...
                if name_match:
                    ...
                    matching_cmpd = Compound.objects.get(id=matching_cmpd_id, project=self.project)
...
def get_kegg_id(cmpd_name):
    ...
    logger.info ('finding an ID for %s' % cmpd_name)
